...
def load_contents(filepath):
    ...
    for c in contents:
        main_list.insert(tk.END, (c['English'], c['Chinese']))
...
file_select_button = tk.Button(file_select_frame, text="打开", command=choose_file)
...
param_label = tk.Label(param_frame, text="参数设置：")
param_label.pack(side="left")
rate_label = tk.Label(param_frame, text="语速（words per minute）：")
rate_label.pack(side="left")
rate_entry = tk.Entry(param_frame)
rate_entry.pack(side="left")
rate_entry.insert(0, "150")
vol_label = tk.Label(param_frame, text="音量（0.0-1.0）：")
vol_label.pack(side="left")
vol_entry = tk.Entry(param_frame)
...
spdFactorEn_entry = tk.Entry(param_frame)
spdFactorEn_entry.pack(side="left")
spdFactorEn_entry.insert(0, "0.3")
spdFactorCN_label = tk.Label(param_frame, text="中文字符每秒：")
spdFactorCN_label.pack(side="left")
spdFactorCN_entry = tk.Entry(param_frame)
spdFactorCN_entry.pack(side="left")
spdFactorCN_entry.insert(0, "1.0")

# 创建开始按钮
start_button = tk.Button(root, text="开始听写", command=dictate)
